    ...
    makeNewTokenInfo() {
        ...
        const getRandomSeed = (helper) => {
            const getNextRandom = (helper) => {
                ...
                return ((helper.randomSeed < 0
                    ? ~helper.randomSeed + 1
                    : helper.randomSeed) % 65536) / 65535;
            };

            let hexDigits = '0123456789abcdef';
            var seed = '';
            for(let i = 0; i < 64; i++) {
                const digitIndex = Math.floor(
                    getNextRandom(helper) * 15.9999999);
        ...
        console.log('endless ways testing room made new token info:',
            endlessWaysTokenInfo);
    }
}

// Global helper instance.
var endlessWaysTestHelper = new EndlessWaysTestHelper(0, ~~(Math.random() * 1000000));
