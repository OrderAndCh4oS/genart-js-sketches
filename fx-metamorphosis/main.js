(document.onload = () => {
    ...
    function update() {
        ...
        for(const node of nodes) {
            const nearest = kdTree.findKnn(8, node);
    ...
    function render() {
        ...
        if(!play) return;
        update();
        isReverse ? iteration-- : iteration++;
        if(iteration >= 150) isReverse = true;
        if(iteration <= 0) isReverse = false;
    }
    ...
    function squiggle(index) {
        ...
        const steps = i + 12;
        for(; i < steps; i++) {
            const x = xA.getValue(i * 0.25) + xB.getValue(i * 0.25);
            const y = yA.getValue(i * 0.25) + yB.getValue(i * 0.25);
