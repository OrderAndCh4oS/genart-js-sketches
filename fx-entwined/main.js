...
if(colourSet.name === 'Constructivist' && !blackAndWhite) {
...
function initialise() {
    ...
    const params = [0.275, 0.425, 0.425, 8, 500, 45, 250, 0.33];
    growthPatterns = [];
    for(let i = 0; i < 20; i++) {
...
function update() {
    ...
    if(iteration % 256 === 0) currentColour = (currentColour + 1) % selectedColoursLength;
    for(let i = iteration % 4; i < growthPatterns.length; i += 4) {
